          ...
          <div className="hidden md:flex items-center gap-1">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="px-3.5 py-1.5 rounded-lg text-sm font-medium text-white/60 hover:text-white hover:bg-white/10 transition-all duration-300"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="px-5 pb-5 pt-2 border-t border-white/10 flex flex-col gap-1">
            {navigation.map((item) => (
              <Link
                ...
                className="px-3 py-2.5 rounded-xl text-[15px] font-medium text-white/80 hover:bg-white/10 transition-colors"
              >
                {item.name}
              </Link>
            ))}
